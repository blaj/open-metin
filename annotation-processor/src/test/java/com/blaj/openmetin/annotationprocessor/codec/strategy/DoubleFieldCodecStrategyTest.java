...
public class DoubleFieldCodecStrategyTest {

  private DoubleFieldCodecStrategy doubleFieldCodecStrategy;
  ...
  public void beforeEach() {
    doubleFieldCodecStrategy = new DoubleFieldCodecStrategy();
  }
  ...
  public void givenNotSupportedFieldType_whenIsSupported_thenReturnFalse(FieldType fieldType) {
    ...
    // when
    var isSupported = doubleFieldCodecStrategy.isSupported(fieldContext);
  ...
  public void givenSupportedFieldType_whenIsSupported_thenReturnTrue() {
    // given
    var supportedFieldType = FieldType.DOUBLE;
  ...
  public void givenValid_whenGenerateDecodingMethod_thenAddCorrectStatement() {
    ...
    // when
    doubleFieldCodecStrategy.generateDecodingMethod(methodSpecBuilder, fieldContext);
    ...
    verify(methodSpecBuilder)
        .addStatement(
            ...
            eq("packet"),
            eq("setHeader"),
            eq(doubleFieldCodecStrategy.getReadMethod()));
  }
  ...
  public void givenValid_whenGenerateEncodingMethod_thenAddCorrectStatement() {
    ...
    // when
    doubleFieldCodecStrategy.generateEncodingMethod(methodSpecBuilder, fieldContext);

    // then
    verify(methodSpecBuilder)
        .addStatement(
            eq("out.$L($L.$L())"),
            eq(doubleFieldCodecStrategy.getWriteMethod()),
            eq("packet"),
            eq("getHeader"));
  }
