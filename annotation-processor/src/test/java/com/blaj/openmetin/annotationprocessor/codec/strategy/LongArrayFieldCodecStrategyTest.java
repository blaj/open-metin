...
public class LongArrayFieldCodecStrategyTest {

  private LongArrayFieldCodecStrategy longArrayFieldCodecStrategy;
  ...
  public void beforeEach() {
    longArrayFieldCodecStrategy = new LongArrayFieldCodecStrategy();
  }
  ...
  public void givenNotSupportedFieldType_whenIsSupported_thenReturnFalse(FieldType fieldType) {
    ...
    // when
    var isSupported = longArrayFieldCodecStrategy.isSupported(fieldContext);
  ...
  public void givenSupportedFieldType_whenIsSupported_thenReturnTrue() {
    // given
    var supportedFieldType = FieldType.LONG_ARRAY;
  ...
  public void given0ArrayLength_whenGenerateDecodingMethod_thenDoNothing() {
    ...
    // when
    longArrayFieldCodecStrategy.generateDecodingMethod(methodSpecBuilder, fieldContext);

    // then
    verify(methodSpecBuilder, times(0)).addStatement(any(), any(), any(), any(), any());
  }
  ...
  public void givenNotUnsigned_whenGenerateDecodingMethod_thenAddCorrectStatement() {
    ...
    verify(methodSpecBuilder)
        .addStatement(
            eq("$L.$L($T.readFixedLongArray(in, $L))"),
            eq("packet"),
            eq("setHeader"),
  ...
  public void givenUnsigned_whenGenerateDecodingMethod_thenAddCorrectStatement() {
    ...
    verify(methodSpecBuilder)
        .addStatement(
            eq("$L.$L($T.readFixedUnsignedIntArray(in, $L))"),
            eq("packet"),
            eq("setHeader"),
            eq(packetCodecUtilsClassName),
            eq(1));
  ...
  public void given0ArrayLength_whenGenerateEncodingMethod_thenDoNothing() {
    ...
    // when
    longArrayFieldCodecStrategy.generateEncodingMethod(methodSpecBuilder, fieldContext);
  ...
  public void givenNotUnsigned_whenGenerateEncodingMethod_thenAddCorrectStatement() {
    ...
    verify(methodSpecBuilder)
        .addStatement(
            eq("$T.writeFixedLongArray(out, $L.$L(), $L)"),
            eq(packetCodecUtilsClassName),
            eq("packet"),
            eq("getHeader"),
            eq(1));
  ...
  public void givenUnsigned_whenGenerateEncodingMethod_thenAddCorrectStatement() {
    ...
    verify(methodSpecBuilder)
        .addStatement(
            eq("$T.writeFixedUnsignedIntArray(out, $L.$L(), $L)"),
            eq(packetCodecUtilsClassName),
            eq("packet"),
            eq("getHeader"),
            eq(1));
